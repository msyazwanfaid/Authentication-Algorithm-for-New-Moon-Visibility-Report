...
print("Alat Optik = Binocular(B) atau Monocular (M) seperti Teleskop? (B/M)")
alatoptikjenis = input()

print("Spesifikasi Teleskop")
print("Masukkan Panjang Fokus Teleskop dalam cm")
teleskopjarakfokus = float(input())

print("Masukkkan Diameter Teleskop dalam cm")
teleskopdiameter = float(input())

print("Masukkan Panjang Fokus Eyepiece yang digunakan dalam cm")
teleskopeyepiecejarakfokus = float(input())

print("Jangkaan Penjagaan Kanta Teleskop, Ttel dari angka 1-10. 1:Kotor 10:Bersih)")
tn = float(input())

print("Jumlah Kanta dan Cermin dalam Teleskop")
tnn = float(input())

print("Refractor atau Reflector (refr/refl)")
jenisteleskop = input()

print("Masukkan Umur Pencerap")
umurpencerap = float(input())
...
for row in table_data:
    print("{: >10} {: >20} {: >10} {: >10} {: >13} {: >18} {: >15} {: >17} {: >24} {: >20} ".format(*row))
while bulanalt >= -0:
...
#Kesan Pengunaan Teleskop
    #Penentuan Monocular Atau Binocular Vision
    if (alatoptikjenis == 'M'):
        fb = 1.41
    else :
        fb = 1

    #Penentuan Cahaya yang Hilang dalam Telescope
    if (jenisteleskop == "refl"):
        dratio = 0.85
    else  :
        dratio = 1
    ft = (1/(pow(tn,tnn)*dratio))

    ##Cahaya yang hilang disebabkan oleh beza saiz diameter mata dengan telescopea
    diametermatapencerap = max(0.2,0.7*math.exp(-0.5*((pow((umurpencerap/100),2)))*(-0.16*(pow(twlpno*(pow(10,(-6))),0.4)))))
    diametermatapencerapstandard = max(0.2,0.68*(math.exp(-0.16*(pow(twlpno*(pow(10,-6)),0.4)))))
    teleskoppembesaran = teleskopjarakfokus/teleskopeyepiecejarakfokus
    fp = max(1,(diametermatapencerapstandard/(teleskoppembesaran*diametermatapencerap)))

    #ratio luas mata manusia dan diameter teleskop
    fa=pow((diametermatapencerapstandard/teleskopdiameter),2)

    #pembetulan bagi cerapan bukan point source (extended source)
    fr=(1+0.03*pow(((teleskoppembesaran*dia)/eyesns),2))*pow(slnrto,2)

    #pembetulan bagi kesan pembesaran teleskop
    fm=pow(teleskoppembesaran,2)

    #Stiles-Crawford Effect
    fsc1 = (1-pow((diametermatapencerapstandard/12.44),4))/(1-pow((teleskopdiameter/(12.44*teleskoppembesaran)),4))
    fsc=min(1,fsc1)

    #Pembetulan kenampakan objek berdasarkan warna
    fc=pow(10,(-0.4*(1-((0.35)/2))))

    #Pembetulan daripada pemupusan atmosfera
    fe=pow(10,(0.48*(difmag/1.25)))

    #Kesan terhadap Cerapan Bintang
    cerapanteleskopsebenarbulannanolambert = bulanmagketno*fb*ft*fp*fa*fsc*fc*fr
    cerapanteleskopsebenarbulanmagnitud = 26.33-2.5*math.log10(cerapanteleskopsebenarbulannanolambert)

    #Kesan terhadap Cerapan Langit
    cerapanteleskopsebenarlangitnanolambert = twlpno*fb*ft*fp*fa*fsc*fc*fm
    ceraapanteleskopsebenarlangitmagnitud = 26.33-2.5*math.log10(cerapanteleskopsebenarlangitnanolambert)

    #Sempadan Kontras Cahaya Teleskop

    contrastteleskop = (abs(cerapanteleskopsebenarbulannanolambert-cerapanteleskopsebenarlangitnanolambert))/cerapanteleskopsebenarlangitnanolambert
    contschteleskop1 = pow((eyesns/dia),2)
    contschteleskop2 = 2.4*pow(cerapanteleskopsebenarlangitnanolambert,-0.1)
    contschteleskop = 0.0028+contschteleskop2*contschteleskop1
    if contrastteleskop>=contschteleskop:
        kenamteleskop = 'Ya'
    else:
        kenamteleskop = 'Tidak'
...
#untuk kegunaan table
    ...
    contrastteleskop =  '%0.2f' %contrastteleskop
    contschteleskop =  '%0.2f' %contschteleskop
    cerapanteleskopsebenarbulanmagnitud = '%0.2f' %cerapanteleskopsebenarbulanmagnitud
    ceraapanteleskopsebenarlangitmagnitud, = '%0.2f' %ceraapanteleskopsebenarlangitmagnitud,
    d = str(lokasi.date)

    table_data = [[d,mataalt,bulanalt,difaz,bulanelon,ceraapanteleskopsebenarlangitmagnitud,cerapanteleskopsebenarbulanmagnitud,contrastteleskop,contschteleskop,kenamteleskop]]
    for row in table_data:
        print("{: >10} {: >10} {: >10}{: >10} {: >15} {: >18} {: >19} {: >25} {: >18} {: >18}  ".format(*row))
    lokasi.date = lokasi.date - ephem.hour * tz
